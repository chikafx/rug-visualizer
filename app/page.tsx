...
function RugDeck({ rugs, loading, error }: { rugs: Rug[]; loading: boolean; error: string }) {
  ...
  function toggleLike() {
    ...
    setLikeError("");
    updateDoc(doc(db, "rugs", currentRug.id), { likes: increment(hasLiked ? -1 : 1) }).catch(error => {
      setLikeError(error instanceof Error ? error.message : "Unable to update this rug's likes.");
    });
